...
fn main() {
    run(future::lazy(|| {
        ...
        let connect = tokio::net::TcpStream::connect(&"127.0.0.1:8080".parse().unwrap())
            .map(move |stream| {
                let (_reader, writer) = stream.split();
                let (tx, rx) = futures::sync::mpsc::unbounded();
                let tx_buf = format!("[Aion] test string...").to_string().into_bytes();
                let _amt = io::write_all(writer, tx_buf);
                println!("Connected");
            })
        ...
        let deadline = tokio::timer::Delay::new(Instant::now() + Duration::from_secs(10))
            .map(|()| {
                println!("10 seconds are over");
            })
